DROP DATABASE IF EXISTS c0225l1;
create database c0225l1;
use c0225l1;
create table jame (
...
SELECT * FROM student
WHERE REVERSE(SUBSTRING_INDEX(REVERSE(student.name), ' ', 1)) IN ('hai', 'huynh');

select * from student
where student.point > 5;

select * from student 
where substring_index(student.name, ' ',1) = 'nguyen';

select
	student.point as points,
    count(*) as so_luong
from student
where student.point between 0 and 10
